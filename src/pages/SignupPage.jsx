...
import { OrbitControls, Float, MeshDistortMaterial, Torus, Box, Stars, Sphere, Octahedron } from '@react-three/drei';
import { Mail, Lock, Eye, EyeOff, UserPlus, User, Sparkles, Shield, ArrowRight, CheckCircle2, Calendar, Users, Zap, ArrowLeft, Home } from 'lucide-react';
...
function MolecularNetwork() {
    ...
    return (
        <group ref={groupRef}>
            ...
            {centerNodes.slice(1).map((node, i) => (
                <mesh key={`line-${i}`} position={[node.pos[0] / 2, node.pos[1] / 2, node.pos[2] / 2]}>
                    <cylinderGeometry args={[0.02, 0.02, Math.sqrt(node.pos[0] ** 2 + node.pos[1] ** 2 + node.pos[2] ** 2), 8]} />
                    <meshStandardMaterial color="#5596e6" metalness={0.8} roughness={0.2} transparent opacity={0.6} />
                </mesh>
            ))}
        </group>
    );
...
function RoleCard({ role, icon: Icon, description, selected, onClick }) {
    return (
        <motion.button
            ...
            className={`relative p-5 rounded-2xl border-2 transition-all ${selected
        ...
        >
            <div className="flex flex-col items-center gap-3">
                <div className={`w-14 h-14 rounded-xl flex items-center justify-center ${selected ? 'bg-brand-200 text-white' : 'bg-slate-100 text-slate-600'
                    }`}>
                    <Icon className="w-7 h-7" />
                </div>
                <div className="text-center">
                    <span className={`font-bold text-lg block mb-1 ${selected ? 'text-brand-200' : 'text-slate-900'}`}>
                        {role}
                    </span>
            ...
            {selected && (
                <motion.div
                    className="absolute -top-2 -right-2 w-7 h-7 bg-brand-200 rounded-full flex items-center justify-center shadow-lg"
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
...
export default function SignupPage({ onNavigateToLogin, onNavigateToHome }) {
        ...
        <div ref={containerRef} className="min-h-screen relative overflow-hidden flex">
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center p-6 lg:p-12 bg-white relative overflow-y-auto">
                ...
                <motion.button
                    ...
                    className="absolute top-6 left-6 flex items-center gap-2 px-4 py-2 text-slate-600 hover:text-brand-200 transition-colors group z-50"
                    whileHover={{ x: -3 }}
                    whileTap={{ scale: 0.95 }}
                        ...
                        <div className="flex items-center gap-3">
                            {[1, 2].map((step) => (
                                <div key={step} className="flex items-center flex-1">
                                    <div className={`flex items-center gap-2 flex-1 ${step < 2 ? '' : ''}`}>
                                    ...
                                    {step < 2 && (
                                        <div className={`h-0.5 w-full mx-2 transition-all ${currentStep > step ? 'bg-green-500' : 'bg-slate-200'
                                            }`} />
                                    )}
                                </div>
                            ))}
