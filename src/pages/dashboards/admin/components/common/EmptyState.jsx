...
function EmptyState({
    ...
    return (
        <motion.div
            ...
            className={`bg-gradient-to-br ${config.bg} border ${config.border} rounded-2xl p-10 text-center`}
        >
            {Icon && (
                <motion.div
                    className={`w-20 h-20 bg-gradient-to-br ${config.iconBg} rounded-2xl flex items-center justify-center mx-auto mb-5 shadow-lg ${config.iconShadow}`}
            ...
            {action && onAction && (
                <motion.button
                    onClick={onAction}
                    className="mt-6 px-6 py-2.5 bg-white border border-slate-200 rounded-xl font-medium text-slate-700 hover:bg-slate-50 transition-colors"
